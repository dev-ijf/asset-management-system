<form method="GET" class="row g-2">
    ...
    <div class="col-md-3">
        ...
        <select name="status" class="form-select">
            ...
            <option value="damaged" @selected(($filters['status'] ?? '') === 'damaged')>Damaged</option>
        </select>
    </div>
    <div class="col-md-3">
        ...
        <select name="location_id" class="form-select">
            ...
            @foreach($locations as $item)
                <option value="{{ $item->id }}" @selected(($filters['location_id'] ?? '') == $item->id)>{{ $item->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-12 d-flex gap-2 mt-2">
        <button type="submit" class="btn btn-primary btn-wave">Terapkan</button>
        <a href="{{ route('reports.audits') }}" class="btn btn-outline-secondary">Reset</a>
    </div>
</form>
